...
```bash
mvn clean install -pl ms-usuarios -DskipTests
```

## Compilar un módulo y sus dependencias

```bash
mvn clean install -pl ms-usuarios -am -DskipTests
```
...
* Implementar pruebas unitarias con JUnit y Mockito para las validaciones financieras.
